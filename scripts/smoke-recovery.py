"""Disposable schema-16 recovery smoke; not a restore tool for existing databases.
...
def snapshot(url):
    with psycopg.connect(url, row_factory=dict_row) as conn:
        ...
        require(versions == [{"version": n} for n in range(1, 17)], "schema16 required")
...
def validate_evidence(before, latest):
    ...
    for evidence in (before, latest):
        ...
        require(evidence["schema_version"] == 16, "schema16 required")
...
def validate_application_evidence(before, latest):
    ...
    require(before["schema_version"] == latest["schema_version"] == 16, "schema16 required")
...
async def recover(admin_url, directory):
    ...
    report = {
        "status": "passed",
        "m2_qualified": False,
        "scope": "schema16-exact-operational-state-application",
        "schema_version": 16,
...
async def main():
    ...
    require(SCHEMA_VERSION == 16, "this bounded smoke is pinned to schema16")
    build_identity()
