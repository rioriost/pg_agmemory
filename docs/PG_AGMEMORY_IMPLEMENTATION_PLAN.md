...
- Document version: 1.18 / Fixed-model agent-memory pilot, 2026-09-25
...
The next **0.3.0.dev1/schema21** increment adds administrator-only persistent
source bindings and notification cursors, atomic read-only lease application,
duplicate/gap fencing and terminal source deletion denial. Its recovery contract
requires exact source state/history, rather than importing changed authority
from an old backup. The real upstream authenticator/notification transport,
...
The signed-notice increment authenticates local RS256 deliveries using a fixed
operator-owned signer and source/reader profile before entering that coordinator.
It validates signature, exact identity, closed claims and bounded delivery time,
not the upstream service's actual ACL state. It does not add network delivery,
remote keys, automatic retries or a new schema. The real connector and delivery/
deletion integration remain required before automatic shared-business retention.
See [signed notices](operations/README.md#signed-source-notice-receiver).
...
Guarded outer COMMIT acknowledgement now has a separate fixed five-second
application budget, independent of server cancel delivery. Expiry remains
...
Native `/v1/` work now shares a 30-second application deadline, with the final
second reserved for errors. Owned connections close before task cancellation,
and COMMIT-time expiry remains non-retryable uncertainty. This bounds application
handling, not backend lifetime or end-to-end network latency.
The v2 owned HA rehearsal observes the five-second guarded COMMIT expiry while
replay is paused, then resumes and reconciles exact replica state read-only
...
The separate v4 lab phase explicitly renews the owned synchronous policy after
replacement/fence prechecks and measures one guarded restricted-writer COMMIT
waiting for replacement replay. Exact state, unchanged original uncertainty and
another host fence check are required; this never authorizes production serving.
The v5 lab adds scoped replication-role HBA rejection and exact sender disconnect,
a guarded unknown COMMIT before admission restoration, and read-only exact-state
reconciliation afterward. It neither retries the write nor qualifies arbitrary
network partitions, blackholes, primary rejoin or production restart.
