...
- Document version: 0.1 / incorporating design review feedback
...
- Status: Implementation is starting. The Git repository was initialized and the original draft was committed before implementation. The phases below do not indicate delivered features; performance and quality figures are acceptance targets, not measured results.
...
**Build pg_agmemory as a Native Memory API that manages the meaning, evidence, time, and permissions of memories, together with its PostgreSQL implementation.** MCP is an adapter for external agents; postgresem is an optional integration. Record ordinary inputs through `observe`, and let background processing generate evidence-backed assertions. At retrieval time, `recall` returns structured data and a context pack within a token budget.
...
Confidence includes `score: 0..1 or null`, `method`, and `calibration_version`. Isolate LLM self-reported values in a separate `extraction_score`. Initially, values not calibrated as probabilities may be null, with authority classification and evidence counts shown instead. Measure Brier score/ECE when adopting calibrated scores. Do not count republications of the same article or reingestion of past summaries as multiple independent pieces of evidence.
...
- For embeddings, record not just model ID but revision, dimensions, distance metric, normalization, and input digest. During re-embedding, keep old and new versions side by side and switch only after coverage and evaluation pass.
...
The guarantee is at-least-once. Before publication, recheck input revisions, access/deletion epochs, source liveness, and lease tokens; discard stale worker results. Make publication idempotent through a dedup key of `job kind + input IDs/revisions + recipe version` and a unique constraint. LLM retry costs cannot also be made exactly-once.
...
Limit LLM proposals to typed candidates. Do not accept arbitrary entity IDs, scopes, ACLs, times, or confidence without validation. Verify evidence such as string spans present in the source, JSON pointers, and tool receipts; send invalid types, insufficient evidence, and ambiguous updates to quarantine.
...
Generate per-run working snapshots triggered by context-window pressure, episode count, or idle time. Start with configurable thresholds, such as 70% of the window, and tune through model-/harness-specific evaluation. Compare the following before and after compaction.
...
Use independent jobs to consolidate similar assertions, reconcile contradictions, and generate summaries. Consolidate only within the same scope, purpose, and time granularity, retaining a mapping from every generated claim to its sources. Even repeated summaries of summaries must remain traceable to leaf episodes.
...
M3 completion requires useful graph recall on real data and passing authorization/deletion tests with at least one of AGE or SQL/PGQ. Simultaneous production operation of both is not mandatory for the MVP.
...
| Optional reranker unavailable | Return base ranking | Send unauthorized candidates to an external reranker |
...
Proceed in dependency order. Durations are initial estimates for a small team of approximately one or two people, not delivery guarantees; staffing and model/provider constraints can change them. Do not ship to production by using feature flags alone to bypass failed safety gates.
...
| M2 / core MVP v0.1 | pgvector+FTS hybrid, Japanese tokenizer, background extraction, same-scope compaction, implicit hook, MCP, job recovery | Pass core quality, authorization, deletion, and recovery gates. Report baselines for at least 20 real-task replays and public evaluations | 4–6 weeks |
| M3 / graph MVP v0.2 | AGE-first graph adapter, bounded expansion, projection rebuild, temporal multi-hop | Demonstrate required functionality with AGE or SQL/PGQ. Complete canonical-oracle conformance and performance/utility evaluation | 3–4 weeks |
| M4 / integration pilot v0.3 | postgresem adapter, source ACL revocation integration, freshness requery, checkpoint harness adapter, operational hardening | Use memory without changing business-query permissions. Correctly propagate partial failures and source deletions | 4–6 weeks |
| M5 / production candidate | Capacity expansion, HA/PITR, tenant isolation, model migration, stable SQL/PGQ adoption decision | Evidence for production-like load, DR, upgrades, and deletion retention. Freeze compatibility contracts | Reestimate after pilot |
...
| Provenance | Leaf sources for all claims, false spans, cycles, source deletion, republication of the same source | Zero unsupported publications, zero cycles, no double counting |
...
| Injection / poisoning | “Ignore previous instructions,” fabricated approvals, false sources, text that attempts to increase importance | Unchanged policy and tool permissions; rejection through evidence validation |
...
### 18.2 Memory Quality Evaluation

Consider [LongMemEval](https://github.com/xiaowu0162/LongMemEval) and [LoCoMo](https://github.com/snap-research/locomo) for public evaluations. For LongMemEval, measure information extraction, multi-session reasoning, temporal reasoning, knowledge updates, and abstention separately. For LoCoMo, compare retrieval and answering over long-term conversations. Neither substitutes for tenant ACL, deletion, or external-side-effect safety tests.

Create an additional internal dataset of at least 500 questions across at least 50 task/session groups. Stratify for Japanese/English, identically named entities, temporal changes, negation, contradictions, stale preferences, prohibited retention, insufficient evidence, source updates, and prompt injection. Do not repurpose real users' histories for benchmarks without permission.

Do not include test questions or answers in synthesis/embedding inputs. Split train/dev/test by chronology and session/entity, avoiding repeated reuse of the test set after parameter tuning. Fix the model, prompt, tokenizer, seed, retrieval K, context budget, dataset commit, and judge rubric.

Comparison baselines are (1) no memory, (2) recent window, (3) full-text context that fits the budget, (4) vector-only, (5) FTS+vector, (6) with temporal/provenance features, and (7) with graphs. Use ablations to judge whether graph, synthesis, and compaction justify their costs. If full text cannot fit, explicitly state that condition rather than presenting a misleading like-for-like comparison.

Measure answer quality with a fixed answer model and report it separately from retrieval quality. Do not rely solely on an LLM judge: score IDs/timestamps/numbers mechanically and have an evaluation subset double-checked by humans. For abstention, report false-assertion rate and answer rate alongside accuracy. Report multiple runs and confidence intervals from session-level bootstrapping.

### 18.3 Metrics and Initial Acceptance Targets

The following are unmeasured design targets. Check feasibility against the M0 baseline; record reasons and old/new values in an ADR when changing them. Do not relax zero-tolerance security/correctness conditions for speed.
...
| Provenance integrity | Rate of published claims traceable to authorized leaf evidence | 100%. Evaluate semantic support separately by hand |
| Assertion precision | Fraction of adopted assertions actually supported by their evidence | At least 95% in a human-reviewed sample. Zero severe fabricated facts |
| Retrieval Recall@20 | Retrieval rate for authorized ground-truth evidence | At least 90% on the internal held-out set; at least as good as the vector-only baseline |
| nDCG@10 / MRR | Ranking quality against relevance labels | No degradation from baseline. Report temporal/update categories separately |
| Update correctness | Rate of selecting current information after corrections while preserving historical queries | 100% for deterministic cases; at least 95% for natural-language fixtures |
| Unsupported assertion rate | Rate of definitive answers despite insufficient evidence | Target at most 2%; zero in critical categories |
| Compaction fidelity | Preservation rate of essential constraints, IDs, and unfinished state | 100% for typed fields; at least 98% in human evaluation of important claims |
| Task continuation | Rate of meeting success conditions in replays before/after compaction | Within 2 percentage points of the uncompressed baseline |
...
| Calibration | Brier/ECE against verified labels | Measure only when adopting numeric confidence. Do not present uncalibrated scores as probabilities |
...
| Cost | LLM/embedding tokens and cost per 1,000 episodes; query cost | Report alongside baseline and stay within tenant limits |
| Footprint | DB/index/WAL/backup bytes per episode/assertion/vector count | Record growth rate and peak during model regeneration |
...
- Also measure E2E including provider time separately. Implicit p95 < 2 s including query embedding is a pilot target; report results per provider.
- Initial synthesis-lag target under the above ingest load is p95 < 60 s; report input length, model, and rate limits.
...
Do not complicate the implementation by assuming it must meet the M profile from the outset. Establish correctness on S first, then add partitioning, batching, index tuning, or worker separation only where M reveals problems.
...
| Synthesis accumulates fabricated facts | Evidence spans, typed candidates, source classes, review/quarantine | Stop automatic publication if the precision gate is unmet |
...
| Model changes alter meaning | Version pinning, shadow evaluation, dual embeddings, rollback | Do not switch if held-out evaluation degrades |
...
Prioritize **authorization, deletion, and evidence → time, concurrency, and recovery → retrieval quality → synthesis/compaction → incremental graph value → integration and scaling**. Implement graph functionality as an M3 requirement, but use it routinely only for queries where ablation shows benefit.
...
- **Evaluation weaknesses**: Do not rely on a single aggregate public benchmark score. Measure Japanese, temporal updates, abstention, authorization, deletion, and recovery separately.
...
Distinguish completion of this plan from completion of implementation. Implementation is starting after Git initialization and the original-draft commit; this plan does not claim verified DDL migration execution, extension compatibility, RLS, APIs, or performance/quality gates. Start with M0 ADRs and golden fixtures, followed by M1's safe vertical implementation. In parallel, establish the public repository, bilingual documentation, MIT license, and Apple Container/Docker test infrastructure.
