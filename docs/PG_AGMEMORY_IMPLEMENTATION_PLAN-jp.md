...
- 文書版: 1.18 / 固定modelのagent-memory pilot、2026-09-25
...
**2026-09-25の製品評価追加:** 固定modelで実agentの保持/忘却、検索、回答を測るpilotを、
上記の基盤gateとは別に実施する。ユーザー選択可能なCopilot生成と独立した合成oracleを使い、
失敗を保持する。撤回した数値閾値は復活させず、一つのpilotで一般的なmodel品質や本番運用を認定しない。
[version付きpilot契約](EVALUATION-jp.md#固定modelのcopilot-agent-memory-pilot2026-09-25)を参照する。
