...
def test_enabled_runner_surfaces_setup_errors_and_waits_for_tcp(tmp_path, engine, failure):
...
"""
    ...
    environment = {
        ...
        if key not in ("PGAG_AGE_PATCHED_IMAGE", "PGAG_AGE_TEST_IMAGE")
    } | {
    ...
    assert result.returncode == (2 if failure == "extension" else 37), result.stderr
